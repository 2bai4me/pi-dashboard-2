...
Verwendet:
  - PyJWT (python-jose) für Token-Erstellung/Validierung
  - bcrypt für Passwort-Hashing
  - argon2 als optional stärkeres Hashing (wenn installiert)
...
from joserfc.errors import InvalidHeaderError, BadSignatureError, InvalidClaimError
...
# Cache für den JWT-Secret-Key (OctKey)
_jwt_key: Optional[OctKey] = None


def _get_jwt_key() -> OctKey:
    """Lazy-Initialisierung des JWT-Keys.
    
    Stellt sicher, dass JWT_SECRET gesetzt ist und ein gültiger Key vorliegt.
    """
    global _jwt_key
    if _jwt_key is not None:
        return _jwt_key
    
    secret = os.getenv("JWT_SECRET") or settings.JWT_SECRET
    if not secret or secret == "change-me-to-a-random-32-byte-base64-secret":
        raise RuntimeError(
            "JWT_SECRET nicht oder unsicher gesetzt! "
            "Setze JWT_SECRET auf einen zufälligen 32-Byte-String (base64-kodiert).\n"
            "Beispiel (PowerShell):\n"
            "  $bytes = [byte[]]::new(32); [Security.Cryptography.RNGCryptoServiceProvider]::new().GetBytes($bytes)\n"
            "  $env:JWT_SECRET = [Convert]::ToBase64String($bytes)"
        )
    
    _jwt_key = OctKey.import_key(secret=secret)
    return _jwt_key
...
async def require_role(
    ...
    """FastAPI-Dependency: Validiert Token + Rolle.
    ...
    Args:
        ...
        credentials: Der Authorization-Header
...
def _get_admin_password_hash() -> str:
    """Lazy-Initialisierung des Admin-Passwort-Hashes.
    
    Beim ersten Aufruf wird das Passwort aus .env gelesen und gehasht.
    """
    ...
    if not password or password == "admin":
        raise RuntimeError(
            "ADMIN_PASSWORD ist unsicher! Bitte setze ein sicheres Passwort in .env:\n"
            "  ADMIN_PASSWORD=<sicheres-passwort-mit-mindestens-12-zeichen>"
        )
    
    _ADMIN_PASSWORD_HASH = bcrypt.hashpw(password.encode(), bcrypt.gensalt()).decode()
    return _ADMIN_PASSWORD_HASH
